...
local runtime_controller = require("ark.runtime_controller")
...
local runtime = runtime_controller.new({
  console = console,
  console_frontend = console_frontend,
  ensure_bridge_runtime = function(...)
    return ensure_bridge_runtime(...)
  end,
  lsp = lsp,
  options = function()
    return options
  ...
  start_session = function(...)
    return start_or_recover_pane_after_runtime_ready(...)
  end,
})
local active_console_status = runtime.active_console_status
local add_candidate_bufnr = runtime.add_candidate_bufnr
local drain_all_ready_waiters = runtime.drain_all_ready_waiters
local ensure_runtime_ready = runtime.ensure_runtime_ready
local is_ark_buffer = runtime.is_ark_buffer
local is_ark_completion_buffer = runtime.is_ark_completion_buffer
local is_ark_runtime_buffer = runtime.is_ark_runtime_buffer
local resolve_bufnr = runtime.resolve_bufnr
local resolve_explicit_help_bufnr = runtime.resolve_explicit_help_bufnr
local resolve_view_source_bufnr = runtime.resolve_view_source_bufnr
local wait_for_repl_ready_for_send = runtime.wait_for_repl_ready_for_send
local wait_until_repl_ready = runtime.wait_until_repl_ready
local with_console_session_ready = runtime.with_console_session_ready
local with_managed_session_ready = runtime.with_managed_session_ready
local with_runtime_ready = runtime.with_runtime_ready
...
local function open_view_tmux_popup(expr, source_bufnr, popup)
  if not should_use_tmux_view_popup() then
    return nil, nil
  end

  popup = type(popup) == "table" and popup or {}
  local server = popup.server or current_nvim_server()
  if type(server) ~= "string" or server == "" then
    if type(popup.on_dispose) == "function" then
      pcall(popup.on_dispose)
    end
  ...
  local popup_opts = vim.tbl_deep_extend("force", view_opts.popup or {}, {
    title = popup.title or ("ArkView: " .. expr),
  })
...
function M.setup(opts)
  ...
  target_actions = target_actions_module.new({
    ...
    lsp = lsp,
    notify = notify,
    open_view_tmux_popup = open_view_tmux_popup,
    options = options,
    refresh = function(...)
      return M.refresh(...)
    end,
...
function M.view(expr, bufnr, view_opts)
  ...
  local view_popup_closed = false
  local function close_view_popup()
    if view_popup_closed then
      return
    end
    view_popup_closed = true
  ...
  local function open_view(runtime_err)
    ...
    if should_use_tmux_view_popup() then
      ...
      opened, open_err = open_view_tmux_popup(expr, source_bufnr, {
        on_dispose = close_view_popup,
        server = console_status and console_status.rpc_socket or nil,
      })
    else
      opened, open_err = view.open({
        expr = expr,
        source_bufnr = source_bufnr,
        options = options,
        lsp = lsp,
        notify = notify,
        on_close = close_view_popup,
      })
    end
    if not opened then
      close_view_popup()
