...
-- Reproduce the user's real-config UI shape: package completion is visible for
-- `library(gg` and a signature-help float is also open. The two floats must not
...
if not result.menu_ready or not result.sig_ready then
...
if overlaps(menu_bounds, signature_bounds) then
  ark_test.fail(vim.inspect({
    error = "signature help overlaps completion menu",
    menu = menu_float,
