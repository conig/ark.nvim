...
-- Reproduce the documentation-first ordering: hover documentation is already
-- visible for `library`, then Blink completion opens near the same cursor. Ark
-- must keep the documentation readable instead of allowing Blink to cover it.
stop_insert_mode()
blink.hide()
vim.o.columns = 80
vim.o.lines = 24
...
local hover_ready = vim.wait(3000, function()
  for _, float in ipairs(float_windows()) do
    local joined = table.concat(float.lines or {}, "\n")
    if float.filetype == "markdown" and joined:find("library", 1, true) then
      return true
    end
  end
  return false
...
if not result.menu_ready or not result.hover_ready then
  ark_test.fail(vim.inspect(result))
end

local menu_float = nil
local hover_float = nil
for _, float in ipairs(result.floats) do
  local joined = table.concat(float.lines or {}, "\n")
  if float.filetype == "blink-cmp-menu" then
    menu_float = float
  elseif float.filetype == "markdown" and joined:find("library", 1, true) then
    hover_float = float
  end
end

if not menu_float or not hover_float then
  ark_test.fail(vim.inspect(result))
end

local menu_bounds = bounds(menu_float)
local hover_bounds = bounds(hover_float)
if overlaps(menu_bounds, hover_bounds) then
  ark_test.fail(vim.inspect({
    error = "hover documentation overlaps completion menu",
    menu = menu_float,
