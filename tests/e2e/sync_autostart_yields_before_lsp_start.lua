...
local original_lsp_prewarm = lsp.prewarm
...
local ok, err = pcall(function()
  ...
  if prewarm_calls ~= 1 then
    error("expected sync autostart to prewarm the detached lsp once, got " .. tostring(prewarm_calls), 0)
  end
...
lsp.prewarm = original_lsp_prewarm
