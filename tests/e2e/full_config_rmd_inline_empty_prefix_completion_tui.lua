...
local session_name = ark_test.register_tmux_session(ark_test.tmux_session_name("rmd_inline_empty_prefix_completion"))
local trace_path = vim.fs.normalize(ark_test.run_tmpdir() .. "/rmd_inline_empty_prefix_completion_trace.log")
local state_home = vim.fs.normalize(ark_test.run_tmpdir() .. "/rmd_inline_empty_prefix_completion_state")
local buffer_path = vim.fs.normalize(ark_test.run_tmpdir() .. "/rmd_inline_empty_prefix_completion.Rmd")
local stop_watchdog = ark_test.start_watchdog(90000, "full_config_rmd_inline_empty_prefix_completion_tui")
...
local ok, err = xpcall(function()
  ...
  tmux({
    ...
    nvim_pane,
    "Escape",
    ":call setline(1, ['---', 'title: \"Inline\"', '---', '', 'The data is `r`.'])",
    "Enter",
    "5G",
    "$",
    "h",
  ...
  ark_test.wait_for("inline mtcars completion", 10000, function()
    completion_show = latest_matching(function(candidate)
      if candidate.label ~= "BlinkCmpShow" or candidate.line ~= "The data is `r `." then
        return false
      end
