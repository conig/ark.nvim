...
local session_name = ark_test.register_tmux_session(ark_test.tmux_session_name("subset_bracket_completion"))
local trace_path = vim.fs.normalize(ark_test.run_tmpdir() .. "/subset_bracket_completion_trace.log")
local state_home = vim.fs.normalize(ark_test.run_tmpdir() .. "/subset_bracket_completion_state")
local buffer_path = vim.fs.normalize(ark_test.run_tmpdir() .. "/subset_bracket_completion.R")
local stop_watchdog = ark_test.start_watchdog(90000, "full_config_subset_bracket_completion_tui")
...
local ok, err = xpcall(function()
  ...
  ark_test.wait_for("subset bracket completion", 10000, function()
    completion_show = latest_matching(function(candidate)
      ...
      if not candidate.line or candidate.line:find("mtcars[", 1, true) ~= 1 then
        return false
      end
      ...
      if trigger.initial_kind == "manual" then
        return false
      end
      if trigger.character ~= "[" and trigger.initial_character ~= "[" then
        return false
      end

      for _, item in ipairs(candidate.items or {}) do
        if item.label == "mpg" and item.client_name == "ark_lsp" then
          return true
        end
      end
