...
local ok, err = pcall(function()
  ...
  if ready ~= nil or type(install_err) ~= "table" or install_err.kind ~= "build_pending" then
    error("expected source-stale user startup to block with build_pending, got " .. vim.inspect({
      ready = ready,
      err = install_err,
    }), 0)
  end
  if #deferred ~= 0 then
    error("expected user startup to avoid deferred freshness path, got " .. vim.inspect(deferred), 0)
  end
  if #jobstart_calls ~= 1 then
    error("expected source-stale user startup to start immediate install, got " .. tostring(#jobstart_calls), 0)
  end
...
vim.print({
  bridge_runtime_user_start_blocks_stale_source = "ok",
})
