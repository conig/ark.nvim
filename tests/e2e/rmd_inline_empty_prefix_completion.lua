...
local test_file = "/tmp/ark_rmd_inline_empty_prefix_completion.Rmd"

vim.fn.writefile({
  "---",
  'title: "Ark inline R empty prefix completion"',
  "---",
  "",
  "The data is `r `.",
}, test_file)
...
local completion_column = assert(line:find("`r ", 1, true)) + 2
...
local mtcars = find_item(items, "mtcars")
if not mtcars then
  fail("empty-prefix inline completion missing mtcars after `r `: " .. vim.inspect(items))
end
